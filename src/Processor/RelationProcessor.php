...
class RelationProcessor implements ProcessorInterface
{
    ...
    protected function addRelation(EloquentModel $model, Relation $relation)
    {
        ...
        if ($foreignColumnName = $relation->getForeignColumnName()) {
            $nameFormatted = str_replace('UF_', '', $foreignColumnName);   // Обрезаем UF_
            $nameFormatted = lcfirst(ucwords(strtolower($nameFormatted), '_')); // Делаем нижний регистр и CamelCase
            $nameFormatted = str_replace('_', '', $nameFormatted);         // Убираем оставшиеся символы "_"
            $relation->setForeignColumnName($nameFormatted);
        }

        if ($localColumnName = $relation->getLocalColumnName()) {
            $nameFormatted = str_replace('UF_', '', $localColumnName);     // Обрезаем UF_
            $nameFormatted = lcfirst(ucwords(strtolower($nameFormatted), '_')); // Делаем нижний регистр и CamelCase
            $nameFormatted = str_replace('_', '', $nameFormatted);         // Убираем оставшиеся символы "_"
            $relation->setLocalColumnName($nameFormatted);
        }
